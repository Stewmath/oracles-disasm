...
for h in xrange(numUniqueTilesetHeaders):
	...
	print hex(address)
	uniqueTilesetHeaderAddresses.append(bankedAddress(uniqueTilesetHeaderBank,address))
...
def parseHeader(address, headerOutput):
	...
	headerOutput.write('\tm_GfxHeader gfx_' + myhex(dat.src,6) + ' ' + wlahex(dat.dest,4) + ' ' + wlahex(dat.size,2))
	...
	if address >= lastAddress:
		...
		while cnt == 0x80:
			if address in gfxHeaderAddresses:
				gfxHeaderOutput.write('gfxHeader' + myhex(toGbPointer(address),4) + ':\n')
	...
	if address >= lastAddress:
		...
		while cnt == 0x80:
			if address in uniqueTilesetHeaderAddresses:
				uniqueTilesetHeaderOutput.write('uniqueTilesetHeader' + myhex(toGbPointer(address),4) + ':\n')
...
for i in xrange(numNpcGraphics):
	...
	if not contained:
		gfxDataList.append(dat)
	npcGfxAddresses.append(dat.src)
#	print str(dat.mode) + ': ' + myhex(dat.src,6)
...
for data in gfxDataList:
	...
	if data.src == 0xad468 or data.src == 0xad5bb or data.src == 0xc92b7:
		...
		# So increase the physical size, and it'll be corrected to its maximum value
...
#	data.printself()
	...
	retData = decompressGfxData(rom,data.src,data.size,data.mode,data.physicalSize)
	outFile.write(retData[1])
	outFile.close()
	# Output compressed
	outFile = open('gfx_precompressed/gfx_' + myhex(data.src,6) + '.bin','wb')
	...
	if data.src != lastAddress:
		if lastAddress != 0:
			gfxDataOutput.write('; Data ends at ' + hex(lastAddress) + ' (physicalSize ' + hex(gfxDataList[i-1].physicalSize) + ')\n\n')
		...
		gfxDataOutput.write('.BANK ' + wlahex(data.src/0x4000,2) + ' SLOT 1\n')
		gfxDataOutput.write('.REDEFINE GFX_CURBANK ' + wlahex(data.src/0x4000,2) + '\n')
		gfxDataOutput.write('.REDEFINE GFX_ADDR ' + wlahex(toGbPointer(data.src),4) + '\n')
		gfxDataOutput.write('.ORGA ' + wlahex(toGbPointer(data.src),4) + '\n')

	if data.src < lastAddress:
		gfxDataOutput.write("; BACKTRACK \n")
#	gfxDataOutput.write('.incbin \"gfx_precompressed/gfx_' + myhex(data.src,6) + '\" ; Type ' + str(data.mode) + '\n')
