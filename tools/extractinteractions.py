		...
		if pos != start and labelPositions.has_key(pos):
			...
			if value == -1:
				...
				for intData in value:
					output += 'group' + str(intData.group) + 'Map' + myhex(intData.map,2) + 'InteractionData:\n'
					intData.address = 0
...
# Go through each group's interaction data, store into data structures for later
...
interactionDataList = sorted(interactionDataList, key=lambda interac:interac.address)
	...
	if address != 0:
		for data2 in interactionDataList:
			if data2.address == address:
				if data2.group == -1:
					mainInteractionDataStr.write('interactionData' + myhex((address&0x3fff)+0x4000,4) + ':\n')
				else:
