...
for i in range(numPaletteHeaders):
        ...
        pointer = bankedAddress(paletteHeaderBank, read16(rom, paletteHeaderTable+i*2))
        header.addr = pointer
        ...
        while repeat:
                ...
                if headerEntry.pointer < 0x8000: # There's 1 entry which points to ram instead of rom
...
for header in sorted(paletteHeaders, key=lambda x:x.addr):
        if header.ref is None:
                outFile.write(header.label + ': ; ' + wlahex(toGbPointer(header.addr),4) + '\n')
                ...
                for j in range(len(header.entries)):
                        ...
                        outFile.write('\t' + macroString + str(headerEntry.startIndex) + ' ')
...
while addr < paletteDataEnd:
        ...
        for i in range(4):
                ...
                green = (word>>5)&0x1f
                blue = (word>>10)&0x1f
                outFile.write('\tm_RGB16 ' + wlahex(red,2) + ' ' + wlahex(green,2) + ' ' + wlahex(blue,2) + '\n')
        outFile.write('\n')
        addr += 8
...
lastHeader = sorted(paletteHeaders, key=lambda x:x.addr)[len(paletteHeaders)-1]
